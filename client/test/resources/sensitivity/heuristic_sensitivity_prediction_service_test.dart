...
void main() {
  group('HeuristicSensitivityPredictionService', () {
    ...
    Future<void> customSetUp({
      ...
      when(foodService.streamFood(any)).thenAnswer((_) => Stream.value(null));
      ...
      for (final edamamFood in mockData.edamamFoods) {
        when(foodService.streamFood(edamamFood.toFoodReference())).thenAnswer((_) => Stream.value(edamamFood));
      }
    ...
    EdamamFoodReference getTestFood({required List<double> concentrations}) {
      ...
      final irritants = [
        for (int i = 0; i < concentrations.length; i += 1)
          Irritant(name: irritantName(i), concentration: concentrations[i])
      ].build();
      final edamamFood = EdamamFood(id: uniqueStr, name: uniqueStr, irritants: irritants);
      when(foodService.streamFood(edamamFood.toFoodReference())).thenAnswer((_) => Stream.value(edamamFood));
    ...
    test('food without irritants has unknown sensitivity', () async {
      ...
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [])), completion(unkP));
    });
    ...
    test('predicts sensitivity of single irritant with simple, dense data', () async {
      ...
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [0])), completion(noneP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [0.5E-5])), completion(noneP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [2.5E-5])), completion(mildP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [4.5E-5])), completion(modP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [6.5E-5])), completion(sevP));

      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [0, 1E-4])), completion(unkP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [0.5E-5, 1E-4])), completion(unkP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [2.5E-5, 1E-4])), completion(mildP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [4.5E-5, 1E-4])), completion(modP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [6.5E-5, 1E-4])), completion(sevP));
    });
    ...
    test('predicts sensitivity of multiple, independent irritants', () async {
      ...
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [0])), completion(noneP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [0.5E-5])), completion(noneP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [1.5E-5])), completion(mildP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [2.5E-5])), completion(modP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [3.5E-5])), completion(sevP));

      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [0, 0])), completion(noneP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [0, 0.5E-5])), completion(noneP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [0, 1.5E-5])), completion(mildP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [0, 2.5E-5])), completion(modP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [0, 3.5E-5])), completion(sevP));

      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [0, 0])), completion(noneP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [0.5E-5, 0])), completion(noneP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [1.5E-5, 0])), completion(mildP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [2.5E-5, 0])), completion(modP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [3.5E-5, 0])), completion(sevP));

      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [1.5E-5, 2.5E-5])), completion(modP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [3.5E-5, 1.5E-5])), completion(sevP));

      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [0, 0, 1E-4])), completion(unkP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [0, 0.5E-5, 1E-4])), completion(unkP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [0, 1.5E-5, 1E-4])), completion(mildP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [0, 2.5E-5, 1E-4])), completion(modP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [0, 3.5E-5, 1E-4])), completion(sevP));
    });
    ...
    test('predicts sensitivity of multiple, mixed irritants', () async {
      ...
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [0.5E-5, 0.5E-5])), completion(modP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [2.5E-5, 2.5E-5])), completion(sevP));
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [3.5E-5, 3.5E-5])), completion(sevP));
    });
    ...
    test('added sensitivities update prediction', () async {
      ...
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [3E-5])), completion(modP));
      ...
      for (final edamamFood in updatedMockData.edamamFoods) {
        when(foodService.streamFood(edamamFood.toFoodReference())).thenAnswer((_) => Stream.value(edamamFood));
      }
      streamController.add(updatedMockData.sensitivityMap);
      await Future.delayed(predictionDelay);
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [3E-5])), completion(mildP));
    });
    ...
    test('updated sensitivities update prediction', () async {
      ...
      streamController.add(mockData.sensitivityMap);
      await Future.delayed(predictionDelay);
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [1.5E-5])), completion(modP));
      ...
      streamController.add(updatedMockData.sensitivityMap);
      await Future.delayed(predictionDelay);
      await expectLater(predictor.predictFromRef(getTestFood(concentrations: [1.5E-5])), completion(sevP));
    });
  });
}
...
class MockData {
  final List<EdamamFood> edamamFoods;
  ...
  MockData._({required this.edamamFoods, required this.sensitivityEntries});
  ...
  factory MockData.from(
    ...
    EdamamFood foodFrom(int foodIndex) {
      final foodName = 'food-$foodIndex';
      final irritants = [
        for (var irrIndex = 0; irrIndex < concentrations.length; irrIndex += 1)
          Irritant(name: irritantName(irrIndex), concentration: concentrations[irrIndex][foodIndex])
      ];
      return EdamamFood(id: foodName, name: foodName, irritants: irritants.build());
    }

    final foods = List.generate(sensitivityLevels.length, (index) => index).map(foodFrom).toList();
    ...
    final sensitivityEntries = [
      for (int i = 0; i < someSensitivities.length; i += 1)
        SensitivityEntry(
          ...
          foodReference: foods[i].toFoodReference(),
          sensitivityLevel: someSensitivities[i],
        )
    ];

    return MockData._(edamamFoods: foods, sensitivityEntries: sensitivityEntries);
  }
}
